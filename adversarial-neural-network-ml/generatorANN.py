...
for nJets in [2,3]:
    ...
    # Import train/test data & generate Adversary training data
    filenameTrain = '../dataset-and-plotting/CSV_differentGenerators/ADV_' + str(nJets) + 'jet_batch_odd.csv'
    dfTrain = pd.read_csv(filenameTrain, index_col=0)
    dfTrainAdversary = dfTrain.loc[dfTrain['Class'] == 0]
    dfTrainAdversary = dfTrainAdversary.reset_index()
    dfTrain['nJ'] = 2

    filenameTest = '../dataset-and-plotting/CSV_differentGenerators/ADV_' + str(nJets) + 'jet_batch_even.csv'
    dfTest = pd.read_csv(filenameTest, index_col=0)
    dfTest['nJ'] = 2
    ...
    def ClassifierTester(classifierNN,df,lam,stage,test,row,nJets):
        ...
        dfSorted.to_csv("GEN_ANN_"+str(nJets)+"Jets.csv")
    ...
    classifierNN.save_weights(str(nJets)+"_"+str(lam)+"_odd.h5")
