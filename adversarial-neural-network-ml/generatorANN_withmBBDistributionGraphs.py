# Monte Carlo Event Generator Aware Adversarial Neural Network Classifier: Includes mBB Distribution Graphs
# Author: Louis Heery
...
for nJets in [2,3]:
    ...
    # Prepare data
    filename_train = '../dataset-and-plotting/CSV_differentGenerators/ADV_' + str(nJets) + 'jet_batch_odd.csv'
    dfTrain = pd.read_csv(filename_train, index_col=0)
    dfTrainAdversary = dfTrain.loc[dfTrain['Class'] == 0]
    dfTrainAdversary = dfTrainAdversary.reset_index()
    dfTrain['nJ'] = 2

    filename_test = '../dataset-and-plotting/CSV_differentGenerators/ADV_' + str(nJets) + 'jet_batch_even.csv'
    dfTest = pd.read_csv(filename_test, index_col=0)
    dfTest['nJ'] = 2

    # Make categorical generator bins
    zTrain = to_categorical(dfTrain['GEN_category'], num_classes=2)
    zTrainAdversary = to_categorical(dfTrainAdversary['GEN_category'], num_classes=2)
    ...
    adversaryLayer = Dense(2, activation="softmax", name='adversary_output')(adversaryLayer)
    ...
    def ClassifierTester(classifierNN,df,lam,stage,test,row):
        predictionScores = classifierNN.predict(df[variables])[:,0]
        df['decision_value'] = ((predictionScores-0.5)*2)
        print ("Iteration Number = " + str(row * 2500))

        figureName = "GEN_ANN_" + str(nJets) + "Jets"+ ".pdf"
        nn_output_plot(df, figureName)

        nameoffile = "df_GEN_ANN_ClassifiedEvents.csv"
        ...
        #### GRAPHS ####
        dfSorted = df.sort_values('decision_value').reset_index()
        dfSorted.to_csv("GEN1_ANN_"+str(nJets)+"Jets.csv")

        ##### Signal & Background Events in LOW NN REGION (-1.0 to +0.8) GRAPHS #####
        ...
        savedSignalData = (dfNEW.loc[dfNEW['Class'] == 1])['mBB_raw'].values #you can also use dfNEW['column_name']
        ...
        # save figure
        figureName = "GEN_ANN_SignalVsBackgroundLowRegion_" + str(nJets) + "Jet_.pdf"
        fig = plt.gcf()
        plt.savefig(figureName, dpi=100, bbox_inches='tight')
        plt.show()
        fig.clear()

        ##### Signal & Background Events in HIGH NN REGION (+0.8 to +1.0) GRAPHS #####
        dfNEW = dfSorted_plus08plus1
        ...
        # save figure
        figureName = "GEN_ANN_SignalVsBackgroundHighRegion_" + str(nJets) + "Jet_.pdf"
        fig = plt.gcf()
        plt.savefig(figureName, dpi=100, bbox_inches='tight')
        plt.show()
        fig.clear()
        ...
        # save figure
        figureName = "GEN_ANN_SignalLowVsHighRegion_" + str(nJets) + "Jet_.pdf"
        fig = plt.gcf()
        plt.savefig(figureName, dpi=100, bbox_inches='tight')
        plt.show()
        fig.clear()
    ...
    row = 1
    ClassifierTester(classifierNN,dfTest,lam,'start',test,row)

    # Initial Training of Adversary ('R')
    classOnAdvNN = Model(input=[inputs], output=[adversaryNN(inputs)])
    optimiserClassOnAdv = SGD(lr=1, momentum=0.5, decay=0.00001)
    classifierNN.trainable = False; adversaryNN.trainable = True
    classOnAdvNN.compile(loss='binary_crossentropy', optimizer=optimiserClassOnAdv, metrics=['accuracy'])
    ...
    for i in range(1,maxLength):
        ...
        modelPerformanceMetrics = combinedNNSystem.train_on_batch(dfTrain[variables].iloc[shuffledIndices], [dfTrain['Class'].iloc[shuffledIndices], zTrain[shuffledIndices]],
                                sample_weight=[dfTrain['training_weight'].iloc[shuffledIndices],dfTrain['adversary_weights'].iloc[shuffledIndices]])
        ...
        if (i%2500 == 0):
            ...
            ClassifierTester(classifierNN,dfTest,lam,str(i),test,row)

    row = int((maxLength/2500) + 2)
    ClassifierTester(classifierNN,dfTest,lam,"end",test,row)
    ...
    nameoffile = "GEN_ANN_" + str(nJets) + "jet.csv"
    df.to_csv(nameoffile)
